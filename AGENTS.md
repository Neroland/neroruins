# Project context for AI coding agents — neroruins

> This and `CLAUDE.md` are kept identical; update both together.

## The mod

- **NeroRuins** — part of the Neroland sci-fi Minecraft mod ecosystem, built on **Neroland Core**.
...
- Mod id: **`neroruins`** (matches the registry namespace + every loader manifest). Package root:
  `za.co.neroland.neroruins`. Author: **Neroland**.
...
- Loader entry points: `NeroRuinsFabric` (+ `NeroRuinsFabricClient`), `NeroRuinsForge`,
  `NeroRuinsNeoForge` — each calls `NeroRuinsCommon.init()` during construction.
- NeoForge/Forge debug tasks use `-PneroruinsDebug`; Fabric Loom honours Gradle `--debug-jvm`.

## IDE (VS Code) run & debug

- Workspace: **`neroruins.code-workspace`** (single-root `"."`). Import the Stonecutter nodes as **static
  ...
  VS Code. Per-node Eclipse project names are `neroruins-<loader>-<mc>`.
...
- This mod has its own **dedicated wiki** in `wiki/` at the repo root: the player- and
  contributor-facing docs for NeroRuins (features, blocks/items, machines, progression, recipes, FAQ).
...
- The wiki is **per-mod** — document only NeroRuins here; cross-mod / ecosystem concepts belong in the
