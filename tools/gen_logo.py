#!/usr/bin/env python3
"""
Generate the NERORUINS mod logo (square), in the shared Neroland family style
...
the exploration/structures mod, so the core-prism glows between two broken BRONZE-stone ruin pillars and a cracked lintel Renders supersampled, then downsamples.

Outputs:
  art/logo/neroruins_logo.png       (1024x1024 master)
  art/logo/neroruins_logo_400.png   (CurseForge/Modrinth-ready)
  common/src/main/resources/neroruins_logo.png  (256x256 in-game mods-list icon)
"""
SEED = 41
MOD = 'neroruins'
NAME = 'NERORUINS'
ACCENT = (200, 152, 88)
ACCENT_BRIGHT = (240, 208, 150)
BRIGHT = (250, 240, 220)
NEBULA = (96, 70, 34)
STAR_TINT = (240, 220, 180)
NAME_GLOW = (200, 152, 88, 255)
TAG_COL = (216, 196, 158, 255)
...
TAG = 'R U I N S   ·   E X P L O R A T I O N   ·   L O R E'

def emblem_frame(base, cx, cy, rad):
    gl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); gd = ImageDraw.Draw(gl)
    colw = rad * 0.42; span = rad * 1.5
    base_y = cy + rad * 1.95; cap_y = cy - rad * 1.35
    for sgn in (-1, 1):
        x = cx + sgn * span
        # weathered pillar with a broken top
        gd.rectangle([x - colw, cap_y + rad * 0.25, x + colw, base_y], fill=STEEL + (255,))
        gd.rectangle([x - colw, cap_y + rad * 0.25, x + colw, base_y], outline=ACCENT + (220,), width=SS)
        # broken chunk
        gd.polygon([(x - colw, cap_y + rad * 0.25), (x + colw, cap_y + rad * 0.25),
                    (x + colw, cap_y - rad * 0.05), (x, cap_y + rad * 0.12),
                    (x - colw, cap_y - rad * 0.1)], fill=STEEL + (255,))
        # capital / base slabs
        for yy in (cap_y + rad * 0.25, base_y - rad * 0.18):
            gd.rectangle([x - colw * 1.35, yy, x + colw * 1.35, yy + rad * 0.2], fill=STEEL_DK + (255,),
                         outline=ACCENT + (220,), width=SS)
    # cracked lintel bridging the tops (with a gap = ruined)
    ly = cap_y
    gd.rectangle([cx - span - colw * 1.3, ly - rad * 0.05, cx - rad * 0.55, ly + rad * 0.3],
                 fill=STEEL_DK + (255,), outline=ACCENT + (220,), width=SS)
    gd.rectangle([cx + rad * 0.55, ly - rad * 0.05, cx + span + colw * 1.3, ly + rad * 0.3],
                 fill=STEEL_DK + (255,), outline=ACCENT + (220,), width=SS)
